function adAstra(str) {
  ...
  let totalCal = 0;
  while (match) {
    // console.log(`Item: ${match.groups.name}, Best before: ${match.groups.exp},
    // Nutrition: ${match.groups.cal}`);
  ...
  while (match) {
    console.log(`Item: ${match.groups.name}, Best before: ${match.groups.exp}, Nutrition: ${match.groups.cal}`);
    totalCal += Number(match.groups.cal);
    match = pattern.exec(str);
  }
}
